...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'accept_friend') {
    ...
    if ($count > 0) {
        $sth_update = $dbh->prepare("UPDATE friends SET status = 'accepted' WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)");
        $sth_update->execute([$userId, $friendId, $friendId, $userId]);
